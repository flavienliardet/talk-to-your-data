...
export async function getDatabricksOAuthToken(): Promise<string> {
  ...
  if (!clientId || !clientSecret || !hostUrl) {
    throw new Error(
      'Set DATABRICKS_CLIENT_ID, DATABRICKS_HOST (and corresponding env).',
    );
  }
  ...
  console.log(
    `[Config] Token acquired, expires in ${expiresInSeconds}s, will refresh in ${
...
export async function getDatabricksUserIdentity(): Promise<string> {
  ...
  try {
    ...
    if (responseHost) {
      ...
      console.log(`[CLI] Host cached: ${responseHost}`);
    }

    console.log(`[CLI] User identity acquired: ${username}`);
    return username;
  } catch (error) {
...
export async function getDatabricksCliToken(): Promise<string> {
  ...
  try {
    const stdout = await spawnWithOutput('databricks', args, {
      errorMessagePrefix:
        'Databricks CLI token failed. Set DATABRICKS_CONFIG_PROFILE or DATABRICKS_HOST.',
    });
    ...
    console.log(
      `[CLI] Token acquired, expires in ${expiresIn}s, ` +
...
export async function getDatabricksToken(): Promise<string> {
  ...
  switch (method) {
    ...
    case 'none':
      throw new Error(
        'No Databricks config. Set one of:\n' +
          '- DATABRICKS_CLIENT_ID + DATABRICKS_HOST\n' +
          '- DATABRICKS_CONFIG_PROFILE or DATABRICKS_HOST',
      );
    default:
...
export async function getDatabaseUsername(): Promise<string> {
  ...
  switch (method) {
    ...
    case 'cli': {
      const pgUser = process.env.PGUSER;
      if (pgUser) {
        console.log(`[CLI] Using PGUSER override for database role`);
        return pgUser;
      }
      console.log(`[CLI] Using user identity for database role`);
...
export function normalizeUserId(id: string): string {
  ...
  const short = trimmed.includes('@') ? trimmed.split('@')[0]!.trim() : trimmed;
...
export async function getUserFromHeaders({
  ...
  let user: User;
  if (forwardedUser) {
    // Databricks Apps environment - use forwarded headers (normalize id for consistency with local/SCIM)
    user = {
      id: normalizeUserId(forwardedUser),
      email:
        forwardedEmail ||

...
export function Chat({
  ...
  const { customInstructions, responseLevel } = useCustomInstructions();
  const responseLevelRef = useRef(responseLevel);
  responseLevelRef.current = responseLevel;
  ...
  const bottomStepRef = useRef<GraphStepData | null>(null);
  ...
  const {
    ...
    transport: new ChatTransport({
      onStreamPart: (part) => {
        ...
        if (partType === 'text-delta' || partType === 'step-start') {
          const ts = Date.now();
          console.log(
            `[GraphStep Client] ts=${ts} received part.type="${part.type}" → clearing graph steps`,
          );
          setGraphSteps([]);
        }
        if (partType === 'text-delta') {
          const current = bottomStepRef.current;
          if (current?.node === 'synthesizer') {
            setBottomStep(null);
          }
        }
      ...
      prepareSendMessagesRequest({ messages, id, body }) {
        ...
        if (isUserMessage) {
          setSuggestedQuestionsFromStream([]);
        }
        ...
        return {
          body: {
            ...
            responseLevel: responseLevelRef.current as 'exploratoire' | 'statistique',
    ...
    onData: (dataPart) => {
      ...
      if (dataPart.type === 'data-graph-step') {
        const step = dataPart.data as GraphStepData;
        const ts = Date.now();
        if (step.node === 'synthesizer') {
          console.log(
            `[GraphStep Client] ts=${ts} label="${step.label}" node="${step.node}" (below-content step)`,
          );
          setBottomStep(step);
          return;
        }
        console.log(
          `[GraphStep Client] ts=${ts} label="${step.label}" node="${step.node}" (received, step count will be updated)`,
        );
        setGraphSteps((prev) => [...prev, step]);
      }
      if (dataPart.type === 'data-suggested-questions') {
        const data = dataPart.data as SuggestedQuestionsData;
        const questions = Array.isArray(data?.suggestedQuestions)
          ? data.suggestedQuestions
          : [];
        console.log('[Chat] data-suggested-questions reçu:', questions.length, questions);
        setSuggestedQuestionsFromStream(questions);
      }
    },
    onFinish: ({
      isAbort,
  ...
  const [thinkingMode, setThinkingMode] = useState<ThinkingMode>(
    initialThinkingMode ?? 'Normal',
  );
  const thinkingModeRef = useRef(thinkingMode);
  thinkingModeRef.current = thinkingMode;

  const [persona, setPersona] = useState<'Stock' | 'Cdistrib'>(
    initialPersona ?? 'Stock',
  );
  const personaRef = useRef(persona);
  personaRef.current = persona;

  const [graphSteps, setGraphSteps] = useState<GraphStepData[]>([]);
  const [bottomStep, setBottomStep] = useState<GraphStepData | null>(null);
  const [suggestedQuestionsFromStream, setSuggestedQuestionsFromStream] =
    useState<string[]>([]);

  useEffect(() => {
    bottomStepRef.current = bottomStep;
  }, [bottomStep]);
  ...
  useEffect(() => {
    console.log(
      '[Chat] status=',
      status,
      'suggestedQuestionsFromStream.length=',
      suggestedQuestionsFromStream.length,
    );
  }, [status, suggestedQuestionsFromStream.length]);
        ...
        <div className="sticky bottom-0 z-1 mx-auto flex w-full max-w-4xl gap-2 border-t-0 bg-background px-2 pb-3 md:px-4 md:pb-4">
          {!isReadonly && (
            <MultimodalInput
              ...
              suggestedQuestionsFromStream={suggestedQuestionsFromStream}
            />
          )}
        </div>
      </div>
